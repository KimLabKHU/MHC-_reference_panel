...
# opt=list(); opt$snpver="ver4"; opt$region="start31.7end32.2_highLDfiltering_50_5_0.8"; opt$haplonetver="ver1_sujung"; opt$qual=20
###############################################################################################
...
head(geno_prob_C4)
C4_GT <- merge(C4_GT, geno_prob_C4[c("SAMPLE", "estimated_geno_matched_typedC4A")] , by="SAMPLE", all.x=TRUE) %>% select(!GT_C4A) %>% rename( GT_C4A=estimated_geno_matched_typedC4A) %>% relocate( GT_C4A, .after=SAMPLE)  
...
if( !dir.exists(  paste0( "/kimlab_wd/yuo1996/C4_HLA_ref/try6_asnpqc_intronrm_with_Eagle_IMPUTE5/KIH_combine/3.phasing_with_Beagle_C4_HLA/",  opt$snpver,"/",opt$region, "QUAL",opt$qual, "_", opt$haplonetver )   )){ 
    dir.create(path= paste0( "/kimlab_wd/yuo1996/C4_HLA_ref/try6_asnpqc_intronrm_with_Eagle_IMPUTE5/KIH_combine/3.phasing_with_Beagle_C4_HLA/",  opt$snpver,"/", opt$region,  "QUAL",opt$qual, "_", opt$haplonetver)  ) } 
...
C4_markers <- c( paste0("C4A_copy", c(2,0,1,3) ),
    ...
    tmpG <- tmp %>%  separate( abc, into=c("G1", "G2"), sep="/")
        ...
        for(j in 1:2){
            C4_insert_for_bgl[ grep("C4A_copy", C4_insert_for_bgl$V1), a ] <-  case_when(
                                                                                ...
                                                                                tmpG[1,j]==0 ~  c( "A", "T", "A", "A"), # copy0
                                                                                tmpG[1,j]==1 ~  c( "A", "A", "T", "A"), # copy1
                                                                                tmpG[1,j]==3 ~  c( "A", "A", "A", "T"), # copy3
                                                                                )

            C4_insert_for_bgl[grep("C4B_copy", C4_insert_for_bgl$V1), a ] <-  case_when(
                                                                                tmpG[2,j]==2 ~  c( "T", "A", "A", "A" ),
                                                                                tmpG[2,j]==0 ~  c( "A", "T", "A", "A"),
                                                                                tmpG[2,j]==1 ~  c( "A", "A", "T", "A"),
                                                                                tmpG[2,j]==3 ~  c( "A", "A", "A", "T"),
                                                                                )
            C4_insert_for_bgl[ grep("HERV_copy", C4_insert_for_bgl$V1), a ] <-  case_when(
                                                                                tmpG[3,j]==2 ~  c( "T", "A", "A", "A", "A" ),  #copy2
                                                                                tmpG[3,j]==0 ~  c( "A", "T", "A", "A", "A"),   #copy0
                                                                                tmpG[3,j]==1 ~  c( "A", "A", "T", "A", "A"),   #copy1
                                                                                tmpG[3,j]==3 ~  c( "A", "A", "A", "T", "A"),   #copy3
                                                                                tmpG[3,j]==4 ~  c( "A", "A", "A", "A", "T"),   #copy4
                                                                                )
...
C4_markers$V2 <- c( rep( 31980581 ,8) , rep(31984684 ,  5) )
C4_markers$V3 <- "A"
C4_markers$V4 <- "T"
colnames(C4_markers)[1] <- "V1"
# C4 position 도 안겹치게 해줘야.
C4_markers$V2[1:8] <- 31980581:(31980581+7)
C4_markers$V2[9:13] <- 31984684:31984688
...
table(whole_bgl[grep("copy", whole_bgl$V2) , ]!=0)
